...
function CreationPreview({ creation, brand, page = 0, compact = false }: { creation: Creation; brand?: BrandProfile; page?: number; compact?: boolean }) {
  ...
  const ratio = `${creation.dimensions.width} / ${creation.dimensions.height}`;
  const colors = brand?.colors ?? ["#2650F6", "#F3E19C"];
  return <article className={`creation-preview preview-${template.style} ${compact ? "compact" : ""}`} style={{ aspectRatio: ratio, "--brand-primary": colors[0], "--brand-secondary": colors[1] } as React.CSSProperties}>
    <div className="preview-head"><span>{brand?.name ?? "Sem marca"}</span><span>{page + 1}/{creation.pages.length}</span></div>
    {template.style === "image" && <div className="preview-image"><Icon name="image" size={compact ? 18 : 30}/><span>{content.imageName || "Imagem da criação"}</span></div>}
    <div className="preview-copy">
      <p className="preview-kicker">{content.subtitle}</p>
      <h3>{content.title}</h3>
      {!compact && <p className="preview-body">{content.body}</p>}
      <span className="preview-cta">{content.cta} <Icon name="arrow" size={14}/></span>
    </div>
...
export function PrismaApp() {
  ...
  }, [theme, retention, customDays, brands, creations]);
  ...
  const newGeneration = (brandId?: string | null | unknown) => { const chosenBrand = typeof brandId === "string" || brandId === null ? brandId : (brands[0]?.id ?? null); setSelectedBrand(chosenBrand); setFormat("portrait"); setTemplateId("message"); setContent(emptyContent); setStep(1); setScreen("generate"); };
  const patchContent = (key: keyof Content, value: string) => setContent((old) => ({ ...old, [key]: value }));

  const generate = async () => {
    if (!content.title.trim()) { setNotice({ tone: "error", text: "Inclua ao menos um título antes de gerar." }); return; }
    setGenerating(true);
    await new Promise((resolve) => window.setTimeout(resolve, 1100));
    const pages = currentTemplate.pages === "multiple" ? [content, { ...content, title: content.subtitle || content.title, subtitle: "Continue explorando", body: content.body, cta: "Continue" }, { ...content, title: content.cta || "Próximo passo", subtitle: "Para encerrar", body: content.body, cta: content.cta || "Saiba mais" }] : [content];
  ...
  const interpretBrief = async () => {
    ...
    try {
      ...
      const data = await response.json() as { slides?: { kicker: string; title: string; text: string }[]; error?: string };
      ...
      setContent((old) => ({ ...old, title: first.title, subtitle: first.kicker, body: first.text, cta: data.slides!.at(-1)?.title ?? old.cta }));
  ...
  const download = () => {
    if (!activeCreation) return;
    const canvas = document.createElement("canvas"); const scale = 1;
    canvas.width = activeCreation.dimensions.width * scale; canvas.height = activeCreation.dimensions.height * scale;
    const context = canvas.getContext("2d"); if (!context) return;
    const data = activeCreation.pages[resultPage] ?? activeCreation.content; const brand = brands.find((item) => item.id === activeCreation.brandId);
    context.fillStyle = brand?.colors[0] ?? "#2650F6"; context.fillRect(0, 0, canvas.width, canvas.height);
    context.fillStyle = brand?.colors[1] ?? "#F3E19C"; context.fillRect(canvas.width * .08, canvas.height * .08, canvas.width * .12, canvas.height * .012);
    context.fillStyle = "#ffffff"; context.font = `600 ${Math.max(36, canvas.width / 15)}px Arial`; context.fillText(brand?.name ?? "Sem marca", canvas.width * .08, canvas.height * .16);
    context.font = `700 ${Math.max(64, canvas.width / 8)}px Arial`; const words = data.title.split(" "); let line = ""; let y = canvas.height * .42;
    words.forEach((word) => { const candidate = `${line} ${word}`.trim(); if (context.measureText(candidate).width > canvas.width * .8) { context.fillText(line, canvas.width * .08, y); line = word; y += canvas.width / 7; } else line = candidate; }); context.fillText(line, canvas.width * .08, y);
    context.font = `500 ${Math.max(30, canvas.width / 28)}px Arial`; context.fillText(data.cta, canvas.width * .08, canvas.height * .9);
    const link = document.createElement("a"); link.href = canvas.toDataURL("image/png"); link.download = `${activeCreation.name.toLowerCase().replace(/[^a-z0-9]+/gi, "-") || "prisma"}-${resultPage + 1}.png`; link.click();
    setNotice({ tone: "success", text: "Página baixada em PNG." });
  };
