...
export function CreatorStudio() {
  ...
  const [brand, setBrand] = useState<Brand>({ name: "Prime Creative", website: "primecreative.com.br", primary: "#0f566b", secondary: "#b9d3d9", font: "Sora", logo: "PC" });
  ...
  function exportSlides() {
    ...
    slides.forEach((slide, index) => {
      ...
      link.download = `prime-creative-${index + 1}.${format === "jpeg" ? "jpg" : "png"}`;
  ...
  return (
    <main className={`app-shell ${darkMode ? "theme-dark" : "theme-light"}`}>
      <aside className="sidebar">
        <a className="wordmark" href="#top" aria-label="Prime Creative Generator"><img src="/prime-creative-logo.png" alt="Prime Creative" /><span><b>Prime Creative</b><small>Generator</small></span></a>
      ...
      {loginOpen && <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="login-modal"><button className="close" onClick={() => setLoginOpen(false)}>×</button><div className="login-logo"><img src="/prime-creative-logo.png" alt="Prime Creative" /></div><p className="eyebrow">PRIME CREATIVE GENERATOR</p><h2>Seu espaço para criar com intenção.</h2><p>O login e a biblioteca persistente serão conectados a um provedor de autenticação antes do lançamento público.</p><button className="primary full" onClick={() => setLoginOpen(false)}>Continuar explorando <span>→</span></button><small>Sem vínculo com uma conta ChatGPT.</small></div></div>}
    </main>
  );
}
